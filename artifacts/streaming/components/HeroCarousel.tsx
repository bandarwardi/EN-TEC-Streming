              ...
              <View style={styles.content}>
                ...
                <View style={styles.actions}>
                  <Pressable 
                    style={({ focused }) => [
                      styles.playButton, 
                      focused && { transform: [{ scale: 1.05 }], borderColor: colors.gold, borderWidth: 2, backgroundColor: 'rgba(255,255,255,0.9)' }
                    ]} 
                    onPress={() => onPlay(item)}>
                    {({ focused }) => (
                      <>
                        <Feather name="play" size={20} color={focused ? colors.gold : "#0A0A0A"} />
                        <Text style={[styles.playButtonText, focused && { color: colors.gold }]}>Play</Text>
                      </>
                    )}
                  </Pressable>
                  ...
                  <Pressable
                    style={({ focused }) => [
                      ...
                      focused && { transform: [{ scale: 1.05 }], borderColor: colors.gold, borderWidth: 2 }
                    ]}
                    onPress={() => handleToggleFavorite(item)}
                  >
                    {({ focused }) => (
                      <>
                        <Feather name={isFav ? 'check' : 'plus'} size={20} color={focused ? colors.gold : (isFav ? colors.gold : '#FFF')} />
                        <Text style={[styles.listButtonText, { color: focused ? colors.gold : (isFav ? colors.gold : '#FFF') }]}>Favorites</Text>
                      </>
                    )}
                  </Pressable>
                  ...
                  <Pressable 
                    ...
                    onPress={() => onInfo(item)}>
                    {({ focused }) => (
                      <Feather name="info" size={20} color={focused ? colors.gold : "#FFF"} />
                    )}
                  </Pressable>
                </View>
              </View>
